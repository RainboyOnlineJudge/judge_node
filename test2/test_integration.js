...
function integration(){
    ...
    describe('test_env',function(){
        it('env.c',function(done){
            var config = JSON.parse( JSON.stringify(inte_config));
            _compile_c( join(t_path,'env.c'),function(exe_path){
                config['exe_path'] = exe_path
                config.input_path = '/dev/null'
                config.output_path = config.error_path = _output_path();
    ...
    describe('test_real_time',function(){
        it('sleep.c',function(done){
            this.timeout(15000);
            var config = JSON.parse( JSON.stringify(inte_config));
            _compile_c( join(t_path,'sleep.c'),function(exe_path){
                config['exe_path'] = exe_path
                judge(config,function(res){
    ...
    describe('test_cpu_time',function(){
        it('while1.c',function(done){
            this.timeout(15000);
            var config = JSON.parse( JSON.stringify(inte_config));
            
            _compile_c( join(t_path,'while1.c'),function(exe_path){
                config['exe_path'] = exe_path
                judge(config,function(res){
    ...
    describe('test_memory1',function(){
        it('memory1.c',function(done){
            this.timeout(15000)
            var config = JSON.parse( JSON.stringify(inte_config));
            config['max_memory']=64*1024*1024;
            _compile_c( join(t_path,'memory1.c'),function(exe_path){
                config['exe_path'] = exe_path
                judge(config,function(res){
                    // malloc succeeded
    ...
    describe('test_memory2',function(){
        it('memory2.c',function(done){
            this.timeout(15000)
            var config = JSON.parse( JSON.stringify(inte_config));
            config['max_memory']=64*1024*1024;

            _compile_c( join(t_path,'memory2.c'),function(exe_path){
                config['exe_path'] = exe_path
                judge(config,function(res){
    ...
    describe('test_memory3',function(){
        it('memory3.c',function(done){
            this.timeout(15000)
            var config = JSON.parse( JSON.stringify(inte_config));
            config['max_memory']=512*1024*1024;

            _compile_c( join(t_path,'memory3.c'),function(exe_path){
                config['exe_path'] = exe_path
                judge(config,function(res){
    ...
    describe('test_re1',function(){
        it('re1.c',function(done){
            this.timeout(15000)
            var config = JSON.parse( JSON.stringify(inte_config));
            _compile_c( join(t_path,'re1.c'),function(exe_path){
                config['exe_path'] = exe_path
                judge(config,function(res){
    ...
    describe('test_re2',function(){
        it('re2.c',function(done){
            this.timeout(15000)
            var config = JSON.parse( JSON.stringify(inte_config));
            _compile_c( join(t_path,'re2.c'),function(exe_path){
                config['exe_path'] = exe_path
                judge(config,function(res){
                    res.result.should.eql(_judger.RUNTIME_ERROR);
    ...
    describe('test_child_proc_cpu_time_limit',function(){
        it('child_proc_cpu_time_limit.c',function(done){
            this.timeout(15000);
            var config = JSON.parse( JSON.stringify(inte_config));

            _compile_c( join(t_path,'child_proc_cpu_time_limit.c'),function(exe_path){
                config['exe_path'] = exe_path
                judge(config,function(res){
                    res.result.should.eql(_judger.CPU_TIME_LIMIT_EXCEEDED);
                    done()
    ...
    describe('test_child_proc_real_time_limit',function(){
        it('child_proc_real_time_limit.c',function(done){
            this.timeout(15000);
            var config = JSON.parse( JSON.stringify(inte_config));


            _compile_c( join(t_path,'child_proc_real_time_limit.c'),function(exe_path){
                config['exe_path'] = exe_path
                judge(config,function(res){
                    res.result.should.eql(_judger.REAL_TIME_LIMIT_EXCEEDED);
    ...
    describe('test_stdout_and_stderr',function(){
        it('stdout_stderr.c',function(done){
            this.timeout(15000)
            var config = JSON.parse( JSON.stringify(inte_config));
            _compile_c( join(t_path,'stdout_stderr.c'),function(exe_path){
                config['exe_path'] = exe_path
                config.output_path = config.error_path = _output_path();
                judge(config,function(res){
                    res.result.should.eql(_judger.SUCCESS);
    ...
    describe('test_uid_and_gid',function(){
        it('uid_gid.c',function(done){
            this.timeout(15000)
            var config = JSON.parse( JSON.stringify(inte_config));
            _compile_c( join(t_path,'uid_gid.c'),function(exe_path){
                config['exe_path'] = exe_path
                config.output_path = config.error_path = _output_path();
                config["uid"] = 65534;
                config["gid"] = 65534;
